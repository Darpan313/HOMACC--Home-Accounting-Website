...
$sum="SELECT SUM(amount) AS amt FROM income";
...
while($row=mysql_fetch_array( $re ))
{
  ...
  echo '<strong> Total-Income: &#x20B9; ' .$amount. '</strong>';
}
...
if($datopt=='TM'){
if($catee=='All'){
  $result = mysql_query("SELECT * FROM income WHERE EXTRACT(MONTH FROM I_date)=EXTRACT(MONTH FROM CURDATE())")

or die(mysql_error());

$sum="SELECT SUM(amount) AS amt FROM income WHERE EXTRACT(MONTH FROM I_date)=EXTRACT(MONTH FROM CURDATE())";
$re=mysql_query($sum);
while($row=mysql_fetch_array( $re ))
{
  $amount=number_format($row['amt'],2);
...
$result = mysql_query("SELECT * FROM income WHERE category='$catee' AND EXTRACT(MONTH FROM I_date)=EXTRACT(MONTH FROM CURDATE())")

or die(mysql_error());

$sum="SELECT SUM(amount) AS amt FROM income WHERE EXTRACT(MONTH FROM I_date)=EXTRACT(MONTH FROM CURDATE()) AND category='$catee'";
$re=mysql_query($sum);
while($row=mysql_fetch_array( $re ))
{
  $amount=number_format($row['amt'],2);
 echo '<br>';
  echo '<strong> Total-Income in '.$catee.' category in this month is: &#x20B9; ' .$amount. '</strong>';
}
...
{
  if($catee=='All'){
  $result = mysql_query("SELECT * FROM income WHERE DATE(I_date) BETWEEN '$sd' AND '$ed'")

or die(mysql_error());

$sum="SELECT SUM(amount) AS amt FROM income WHERE DATE(I_date) BETWEEN '$sd' AND '$ed'";
$re=mysql_query($sum);
while($row=mysql_fetch_array( $re ))
{
  $amount=number_format($row['amt'],2);
  echo '<br>';
  echo '<strong> Total-Income in all categories from <'.$sd.'> to <'.$ed. '>is: &#x20B9; ' .$amount. '</strong>';
}


}
else{
$result = mysql_query("SELECT * FROM income WHERE category='$catee' AND DATE(I_date) BETWEEN '$sd' AND '$ed'")

or die(mysql_error());

$sum="SELECT SUM(amount) AS amt FROM income WHERE DATE(I_date) BETWEEN '$sd' AND '$ed' AND category='$catee'";
$re=mysql_query($sum);
while($row=mysql_fetch_array( $re ))
{
  $amount=number_format($row['amt'],2);
 echo '<br>';
  echo '<strong> Total-Income in '.$catee.' category from <'.$sd.'> to <'.$ed. '>is: &#x20B9; ' .$amount. '</strong>';
}
...
$result = mysql_query("SELECT * FROM income WHERE category='$catee'")
...
echo "<thead><tr class= main-heading > <th>DATE</th> <th>AMOUNT(Rs)</th><th>CATEGORY</th><th>DESCRIPTION</th> <th>EDIT</th> <th>DELETE</th></tr></thead>";
...
<br><strong>AMOUNT: *</strong> <input type="text" name="amount" value="<?php echo $amount; ?>" /><br/><br/>
...
mysql_query("INSERT income SET I_date= now(), amount='$amount',category='$category',description='$description'")
